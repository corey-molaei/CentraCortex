...
class SharePointConnector(Base):
    __tablename__ = "sharepoint_connectors"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    tenant_id: Mapped[str] = mapped_column(String(36), ForeignKey("tenants.id", ondelete="CASCADE"), unique=True)
    azure_tenant_id: Mapped[str] = mapped_column(String(128), nullable=False)
    client_id: Mapped[str] = mapped_column(String(255), nullable=False)
    client_secret_encrypted: Mapped[str] = mapped_column(String(2048), nullable=False)
    site_ids: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False)
    drive_ids: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False)
